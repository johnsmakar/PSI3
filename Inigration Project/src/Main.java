			...
			while (enemies[indexOfRandomEnemy].getHealth() >= 0) {
				...
				case "2":
					...
					if (shield > 0) {
						...
						if (shield < 0) {
							shield = 0;
							continue; //if and when the shield has reached 0, the damage will continue and go to affect the health
						}
					} else {
						health -= damageTaken;
					}
				...
				case"3":
					...
					for( int i=0; i<grid.length; i++) {
					    if(grid[i][i]==target){
					        ...
					        break; //if the grid location matches the target it will skip the second dimension of the array
					    }
					for(int j=0; j<grid[i].length; j++) {
						if(grid[i][j]==target){ // the == operator checks if the integer[][] is equal to the integer(target)
