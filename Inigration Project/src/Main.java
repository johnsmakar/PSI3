...
/*
 * A program integrating skills learned in COP 2006 This program is a text-based
 * RPG game inspired by,Tom Clancy'sThe Division 2 Game Alpha 0.01
 */
...
//Boolean - Boolean data type is used for logical values such as true or false. Boolean is the type required by the conditional expressions used in control statements. 
//Byte - An 8-bit signed two's complement integer. It has a minimum value of -128 and a maximum value of 127 (inclusive). 
//Char - a single two-byte (16-bit) Unicode character. Short - An 16-bit signed two's complement integer. It has a minimum value of -32,768 and a maximum value of 32,767 (inclusive). 
//Int - A 16-bit signed two's complement integer.
//Long - a 64-bit two's complement integer. The signed long has a minimum value of -263 and a maximum value of 263-1. 
...
public class Main {
	...
	public static void main(String[] args) {
		...
		final int DAMAGE = 75; // final variable cannot be reassigned a value
		...
		case "y":
			...
			String dateOfBirth = (String) JOptionPane.showInputDialog(null, "Date of Birth", "Dialog Title", 2, null,
					null, "MM/DD/YYYY");
		...
		ArrayList<Integer> killCounter = new ArrayList<Integer>(); // creates ArrayList
		/*
			...
			while (enemies[indexOfRandomEnemy].getHealth() >= 0) {
				...
				case "2":
					...
					if (shield > 0) {
						...
						if (shield < 0) {
							shield = 0;
							continue; // if and when the shield has reached 0, the damage will continue and go to
										// affect the health
						}
					} else {
						health -= damageTaken;
				...
				for (int i = 0; i < grid.length; i++) {
					for (int j = 0; j < grid[i].length; j++) {
						if (grid[i][i] == target) {
							...
							if (grid[i][j] == target) { // the == operator checks if the integer[][] is equal to the
								// integer(target)
